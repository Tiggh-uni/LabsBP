# ========== main.py (KIVY VERSION - С ИЗМЕНЕНИЕМ КОЛИЧЕСТВА ИНГРЕДИЕНТОВ) ==========
...
class RecipeCalculator:
    ...
    def get_recipe_ingredients(self, recipe_name: str) -> Dict[str, float]:
        """Возвращает базовый состав рецепта"""
...
class EditIngredientsPopup(Popup):
    """Всплывающее окно для изменения количества ингредиентов"""
    
    def __init__(self, recipe_name: str, ingredients: Dict[str, float], callback, **kwargs):
        super().__init__(**kwargs)
        self.title = f"Изменить количество ингредиентов: {recipe_name}"
    ...
    def _build_ui(self):
        """Построение интерфейса окна редактирования"""
        main_layout = BoxLayout(orientation='vertical', spacing=10, padding=10)
        
        # Пояснительная надпись
        info_label = Label(
            text="[i]Измените количество ингредиентов (в граммах)[/i]",
            markup=True,
            size_hint_y=0.08
        )
        main_layout.add_widget(info_label)
        
        # Область с полями ввода (ScrollView)
        scroll = ScrollView(size_hint_y=0.8)
        grid = GridLayout(cols=2, spacing=10, size_hint_y=None)
        grid.bind(minimum_height=grid.setter('height'))
        
        # Создаём поля для каждого ингредиента
        for ing_name, default_grams in self.original_ingredients.items():
            ...
            input_field = TextInput(
                ...
                input_filter='float',  # только числа и точка
                multiline=False,
                size_hint_x=0.6
            )
    ...
    def _on_ok(self, instance):
        """Сбор введённых данных и передача обратно"""
        custom_weights = {}
        for ing_name, input_field in self.inputs.items():
            try:
                ...
                # Если введено не число, используем исходное значение
...
class RecipeAppLayout(BoxLayout):
    ...
    def _build_ui(self):
        ...
        # Кнопка расчёта с базовыми ингредиентами
        ...
        self.result_label.bind(
            width=lambda _, w: self.result_label.setter('text_size')(self.result_label, (w, None)),
        ...
        # Кнопки экспорта
        export_title = Label(text="[b]Экспорт отчёта[/b]", markup=True, size_hint_y=0.05)
        self.add_widget(export_title)
        
        export_layout = BoxLayout(orientation='horizontal', size_hint_y=0.08, spacing=10)
    ...
    def _calculate(self, instance):
        """Обработчик расчёта с базовыми ингредиентами"""
    ...
    def _open_edit_popup(self, instance):
        """Открыть окно редактирования ингредиентов"""
        recipe = self.recipe_spinner.text
        if recipe == "Выберите рецепт":
            self._show_popup("Ошибка", "Сначала выберите рецепт")
            return
        
        # Получаем базовый состав рецепта
    ...
    def _calculate_with_custom(self, custom_weights: Dict[str, float]):
        ...
        try:
            self.current_result = self.calculator.calculate(self.current_recipe or self.recipe_spinner.text, custom_weights)
            self.result_label.text = self.current_result.to_text()
            self.result_label.markup = True
        except Exception as e:
            self._show_popup("Ошибка расчёта", str(e))
