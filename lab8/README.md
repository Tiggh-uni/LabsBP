Цель: Реализуйте Веб-скрейпинг новостных заголовков с Tkinter по своему варианту. Можно изменить задание на собственную тему, согласовав с преподавателем. Требования:
...
В ходе работы разработано приложение для веб-скрейпинга новостных заголовков с графическим интерфейсом на основе библиотеки Tkinter, реализованное с соблюдением принципов объектно-ориентированного программирования. Приложение выполняет HTTP-запросы к заданному URL (например, BBC News), парсит HTML-страницу с помощью BeautifulSoup и извлекает заголовки новостей вместе со ссылками. Для обработки ошибок сети, некорректного URL и проблем с парсингом создана иерархия собственных классов исключений (NetworkError, InvalidURLError, ParsingError). Результаты отображаются в таблице с возможностью сохранения в текстовый файл через диалоговое окно, а длительные операции выполняются в отдельном потоке для сохранения отзывчивости интерфейса. Таким образом, получено полностью функциональное кроссплатформенное приложение, демонстрирующее применение ООП, обработку исключений и взаимодействие с веб-страницами.
